    ...
    run:async()=>{
        try {
          ...
          if (Number(menu.val) < 50) {
            await messageSession.update(sessionId,'airtimeSelfMessage','The minimum amount is 50 Naira')
            return 'airtimeSelf'
          }
          ...
          await userSession.update(sessionId,'airtimeSelfMessage','Insufficient funds. \n')
          return 'airtimeSelf'
        } catch (err) {
